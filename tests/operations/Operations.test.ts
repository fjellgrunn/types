...
import { isComKey, isPriKey, type OperationParams, type Operations } from '../../src/operations/Operations';
...
describe('Operations Interface', () => {
  it('should compile with correct type parameters', () => {
    ...
    const _userOps: Operations<
      { kt: 'user', pk: string, name: string, events: any },
      'user'
    > | null = null;
    
    const _commentOps: Operations<
      { kt: 'comment', pk: string, loc: [{ kt: 'post', lk: string }], text: string, events: any },
      'comment',
      'post'
    > | null = null;
